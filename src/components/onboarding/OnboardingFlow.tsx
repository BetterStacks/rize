...
export default function OnboardingFlow({ resumeId }: OnboardingFlowProps) {
    ...
    const autoCreateProfile = async () => {
      ...
      if (
        ...
        try {
          ...
          console.log(
            "🔍 Claimed username from client cookie:",
            claimedUsername
          );
  ...
  const onComplete = async () => {
    try {
      await bulkInsertSections(session?.data?.user?.profileId as string);
      ...
      if (formData?.resumeData) {
        ...
        if (result.success) {
          toast.success(
            `Resume data imported: ${result.stats?.experience || 0} experiences, ${result.stats?.education || 0} education entries`
          );
        } else {
          toast.error(
            "Failed to import resume data, but profile was created successfully"
          );
        }
        ...
        try {
          ...
          if (result.ok) {
            ...
            if (parseResult.success) {
              toast.success(
                `Resume parsed: ${parseResult.stats?.experience || 0} experiences, ${parseResult.stats?.education || 0} education entries`
              );
            }
          } else {
