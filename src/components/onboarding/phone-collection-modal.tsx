...
import { initiateOnboardingCall, skipOnboarding } from '@/actions/onboarding-actions'
...
export default function PhoneCollectionModal({ isOpen, userName, onClose }: Props) {
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="sm:max-w-md">
                ...
                <DialogHeader>
                    <DialogTitle className='font-medium tracking-wide'>Welcome, <span className='font-normal tracking-tight'>{userName}</span> 👋</DialogTitle>
                </DialogHeader>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div className="flex h-10 w-full items-center rounded-md border border-gray-300 bg-background px-3 py-2 text-sm">
                            <PhoneInput
                                ...
                                countrySelectProps={{ className: 'w-24 focus:outline-none bg-transparent' }}
                                numberInputProps={{
