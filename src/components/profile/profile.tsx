            ...
            <div className=" max-w-2xl w-full flex flex-col items-start justify-start">
                ...
                <div className="flex mt-2 text-gray-700 dark:text-gray-300 justify-start items-center">
                    ...
                    <span className='text-sm font-medium tracking-tight md:text-base'>
                        Joined at {(data?.createdAt as Date)?.toLocaleDateString("en-US", { day: "numeric", month: "short", year: "numeric" })}
                    </span>

                    {data?.website && (
                        <>
                            <span className="mx-2">•</span>
                            <Link target="_blank" href={data?.website} className="inline-flex tracking-tight items-center justify-start text-sm md:text-base font-medium hover:underline underline-offset-2 cursor-pointer">
