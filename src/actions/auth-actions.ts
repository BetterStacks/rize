...
export async function createProfileAfterOAuth(
  ...
  console.log("🔍 createProfileAfterOAuth called with:", {
    userId,
    userInfo,
    claimedUsername,
  });
  ...
  try {
    ...
    if (existingProfile.length > 0) {
      ...
      return {
        ...
        username: existingProfile[0].username,
      };
    }
    ...
    if (claimedUsername) {
      if (isUsernameReserved(claimedUsername)) {
        ...
        let baseUsername = claimedUsername
          .toLowerCase()
          .replace(/[^a-z0-9]/g, "")
          .substring(0, 15);
    ...
    console.log(
      `✅ Created profile for ${userInfo.name} with username: ${finalUsername}`
    );
...
export const getEnrichedSession = async () => {
  try {
